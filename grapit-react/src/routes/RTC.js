...
function RTC({ chat }) {
  // let [localVideo, setLocalVideo] = useState(useRef());
  // let [remoteVideo, setRemoteVideo] = useState(useRef());
  ...
  let localUserName = useSelector(state => state.user.nickName);
  let localRoom = chat.roomId;
  ...
  function getMedia(constraints) {
    if (localStream) {
      localStream.getTracks().forEach(track => {
        console.log("I don't want this FUNC ! CHECK");
        track.stop();
      });
    }
  ...
  function createPeerConnection() {
    ...
    console.log('createPeerConnection success');
  }
  ...
  function handleICEConnectionStateChangeEvent() {
    ...
    if (status === 'connected') {
      log('status : ' + status);
      // !!TODO : remote_video show 상태로로, 끊졌을때는 remote_video 를 hide 상태로 변경
      // $("#remote_video").show();
  ...
  return (
    <div>
      ...
      {/*</div>*/}
      {/*<div className="col-lg-6 mb-3">*/}
      {/*    <video id="remote_video" autoPlay playsInline/>*/}
      {/*</div>*/}
